...
function getBoxInfo(maxWidth: number, maxHeight: number, style: any) {
...
const possibleDropdowns = ['normal', 'center', 'fit', 'corner']
const centralPositions = ['top', 'bottom', 'left', 'right']
const numericProps = [
  ...centralPositions, 'maxWidth', 'maxHeight', 'width', 'height'
]
const cornerPositions = [
  'top_left', 'top_right', 'bottom_left', 'bottom_right'
]
const possiblePositions = [...centralPositions, ...cornerPositions]

class RowiDropdown extends RowiElement {
  #debouncedUpdate
  #overlayClicked
  private _opened: boolean
  private _holderLastRect: {}
  private _dropdownLastRect: DOMRect
  private _positions: Set<string>
  private _holder: any
  private _debouncedUpdateTimeoutID: NodeJS.Timeout
  private _holderRectCheck: NodeJS.Timer
  private _dropdownRectCheck: NodeJS.Timer
  private _arrowPositions: { x: number; y: number }
  private _largestSize: number
  private _largestBoxPosition: any
  private _largestBoxStyle: any
  ...
  get props() {
    this._positions = new Set(possiblePositions)
    this._holder = null
    ...
    return {
      ...
      dropdownStyle: {
        ...
        validator(value: string) {
          return possibleDropdowns.includes(value)
        }
      },
      dropdownPosition: {
        type: 'string', default: 'all',
        validator(valuesString: string) {
          valuesString = valuesString.trim()
          if (valuesString === 'all') return true
          const values = valuesString.split(/ +/)
          for (const value of values) {
            if (!possiblePositions.includes(value)) return false
          }
          return true
        },
        handler({ newValue }) {
  ...
  #_overlayClicked(event: MouseEvent) {
  ...
  #applyBoxStyle(style) {
    ...
    Object.entries(style).forEach(([prop, value]) => {
      if (numericProps.includes(prop)) value += 'px'
